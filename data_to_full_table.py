...
def extract_data_table(gamerName, matches):
    extracted_list = []  # 데이터를 저장할 리스트
    for match in matches:  # 각 경기마다 반복
        match_data = {}  # 경기 데이터를 담을 딕셔너리
...
full_table = []  # 최종 데이터를 저장할 리스트

# 각 게이머에 대해 반복
for j in range(len(df)):
    gamerName = df.iloc[j]['GamerName']  # 게이머 이름
    matches = df.iloc[j]['matches']  # 해당 게이머의 경기들
    gamertable = extract_data_table(gamerName, matches)  # 경기 데이터 추출
    full_table.extend(gamertable)  # 추출된 데이터를 전체 리스트에 추가
