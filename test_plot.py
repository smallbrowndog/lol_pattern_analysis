...
df_match = df
time = pd.to_datetime(df_match['gameCreation']/1000, unit='s')
...
axes[1, 0].set_title('Damage Per Minute (after 14m)')
axes[1, 0].grid(True)
axes[1, 1].scatter(time, data_af14_dtpm, c=colors, s=8)
axes[1, 1].set_title('DamageTaken Per Minute (after 14m)')
axes[1, 1].grid(True)
